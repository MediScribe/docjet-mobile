...
## 16. [ ] **Fix Mock Server Auth Provider Integration**
    ...
    16.2. [ ] **RED**: Write failing tests for provider overrides
       - Write a test in `test/core/app/provider_override_test.dart` that verifies provider overrides
       - Test should verify that the correct provider (from auth_notifier.g.dart) is properly overridden
       - Create a TestApp with ProviderScope that uses real overrides from main.dart
       - Assert that the provider doesn't throw the UnimplementedError when accessed
       - Verify the test fails because we're currently overriding the wrong provider
    
    16.3. [ ] **GREEN**: Fix provider overrides correctly
       - Remove the duplicate provider definition from main.dart
       - Import the correct generated provider from auth_notifier.dart
       - Update ProviderScope in main.dart to override only the generated provider
       - Update auth_module.dart to use the correct provider reference
       - Verify the fix works by running the test and with the mock server
