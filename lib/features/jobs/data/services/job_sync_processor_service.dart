...
class JobSyncProcessorService {
  ...
  Future<Either<Failure, Unit>> processJobDeletion(Job jobToDelete) async {
    ...
    if (jobToDelete.serverId != null) {
      try {
        _logger.d(
          'Deleting job on server (serverId: ${jobToDelete.serverId})...',
        );
        await _remoteDataSource.deleteJob(jobToDelete.serverId!);
        _logger.i(
          'Successfully deleted job on server (serverId: ${jobToDelete.serverId}).',
        );
        remoteDeleteSuccess = true;
      } catch (e) {
        _logger.e(
          'Failed to delete job on server (serverId: ${jobToDelete.serverId}): $e. Marking for retry or failure.',
        );
    ...
    if (remoteDeleteSuccess || jobToDelete.serverId == null) {
      _logger.d(
        'Proceeding with permanent local deletion for ${jobToDelete.localId}.',
      );
      ...
      _logger.w(
        'Skipping permanent local deletion for ${jobToDelete.localId} due to remote failure. Returning Left.',
      );
  ...
  Future<Either<Failure, Unit>> _permanentlyDeleteJob(String localId) async {
    ...
    try {
      ...
      _logger.w(
        'CacheException getting job $localId (Might be already deleted?): $e',
      );
    ...
    try {
      ...
      _logger.e(
        'CacheException during DB delete operation for job $localId: $e',
      );
      return Left(CacheFailure('Failed to delete job $localId from DB: $e'));
    } catch (e) {
      // Catch any other unexpected error during delete
      _logger.e(
        'Unexpected error during permanent local deletion DB operation for job $localId: $e.',
      );
      return Left(
