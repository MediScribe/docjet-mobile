...
void main(List<String> arguments) async {
  ...
  try {
    ...
    _printTestResults(failedTests, debugMode, exceptMode, events);
...
void _printUsage(ArgParser parser) {
  print(
    'Usage: ./scripts/list_failed_tests.dart [--debug] [--except] [test_target]',
  );
  print('');
  print(
...
Map<String, List<Map<String, dynamic>>> _extractFailedTests(
  ...
  for (final event in allEvents) {
    // Look for tests that completed with failure OR error status
    if (event['type'] == 'testDone' &&
      ...
      // Combine the testDone event with any associated error event
      final failureDetails = {
        'name': testName,
        'id': testId,
        'failureEvent': event, // This is the testDone event
        'errorEvent': errorEventsById[testId], // May be null
      };
      ...
      if (_debugScript) {
        print(
          '[SCRIPT_DEBUG] Found failed/error test: [$filePath] $testName (ID: $testId, Result: ${event['result']})',
        );
      }
    }
  }
...
void _printTestResults(
  ...
  if (failedTestsByFile.isEmpty) {
    ...
    if (exceptMode) {
      // If exceptMode is true, print only exceptions
      _printExceptionDetails(failedTestsByFile);
    } else {
      // Otherwise, print the standard details
      ...
      // Add hint about --except
      print(
        '\x1B[33mTip: Run with --except to see only exception details.\x1B[0m',
      );
    }
  }
}
...
// --- New function to print only exception details ---
void _printExceptionDetails(
  Map<String, List<Map<String, dynamic>>> failedTestsByFile,
) {
  print('');
  print('\x1B[31m--- Failed Test Exceptions ---\x1B[0m'); // Red header
  ...
  for (final filePath in sortedFiles) {
    ...
    for (final failure in tests) {
      final testName = failure['name'] as String;
      final failureEvent =
          failure['failureEvent'] as Map<String, dynamic>; // testDone
      final errorEvent =
          failure['errorEvent']
              as Map<String, dynamic>?; // Explicit error event

      // Prioritize details from errorEvent if it exists, otherwise use failureEvent
      final String? error =
          (errorEvent?['error'] as String?) ??
          (failureEvent['error'] as String?);
      final String? stackTrace =
          (errorEvent?['stackTrace'] as String?) ??
          (failureEvent['stackTrace'] as String?);

      if (error != null || stackTrace != null) {
        exceptionsFound = true;
        print(''); // Separator
        print(
          '\x1B[33mException in Test:\x1B[0m $testName (\x1B[90m$filePath\x1B[0m)', // Yellow marker
        );
        if (error != null) {
          // Print error, indenting multi-line messages
          final errorLines = error.split('\n');
          print('  \x1B[31mError:\x1B[0m ${errorLines.first}');
          for (int i = 1; i < errorLines.length; i++) {
            print('         ${errorLines[i]}');
          }
        } else {
          print('  \x1B[31mError:\x1B[0m (No error message provided)');
        }
        if (stackTrace != null) {
          print('  \x1B[90mStack Trace:\x1B[0m'); // Dim color for stack trace
          // Print stack trace, indenting each line
          stackTrace.split('\n').forEach((line) {
          ...
          print('  \x1B[90mStack Trace:\x1B[0m (No stack trace provided)');
        }
      } else {
        // This case should be rare if result is failure/error
        print(''); // Separator
        print(
          '\x1B[33mFailed Test (No Exception Details):\x1B[0m $testName (\x1B[90m$filePath\x1B[0m)',
        );
        print('  (Could not find error or stack trace in test event data)');
      }
    }
  }
  ...
  if (!exceptionsFound) {
    print('');
    print('No specific exception details found in the failed test events.');
  }

  print('');
  print('\x1B[31m--- End of Exceptions ---\x1B[0m'); // Red footer
}
